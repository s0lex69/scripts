...
Rubick.levelColorOption = Menu.AddOptionCombo({  "Hero Specific", "Rubick","Advanced" }, "Display Level Color", {"Green", "Yellow", "Red", "Blue", "White", "Black"}, 0)
Rubick.pickedSkills ={}
...
function Rubick.OnUpdate()
    ...
    if skillTarget and not NPC.IsChannellingAbility(myHero) then
        ...
        if currentSkill and Rubick.pickedSkills[Ability.GetName(currentSkill)] then
            if (Rubick.findIndex(currentSkill)>candidateOrder or (Ability.GetCooldownTimeLeft(currentSkill)>8 and Rubick.findIndex(currentSkill)~=candidateOrder)) and NPC.IsVisibleToEnemies(myHero) then
                Ability.CastTarget(ultimate, target)
                Rubick.TimeTick = GameRules.GetGameTime() +2
            end 
        else
            if NPC.IsVisibleToEnemies(myHero) then 
                Ability.CastTarget(ultimate, target)
                Rubick.TimeTick = GameRules.GetGameTime() +2
            end
        end 
...
function Rubick.DrawAbilitySquarePriorityQueue(ability, x, y, index)
   ...
   -- local castable = Ability.IsCastable(ability, NPC.GetMana(hero), true)
    ...
    Renderer.SetDrawColor(imageColor[1], imageColor[2], imageColor[3], 255)
    Renderer.DrawImage(imageHandle, realX, y/1.5, Rubick.boxSize, Rubick.boxSize)

    Renderer.SetDrawColor(outlineColor[1], outlineColor[2], outlineColor[3], 255)
    Renderer.DrawOutlineRect(realX, y/1.5, Rubick.boxSize, Rubick.boxSize)
    ...
    Rubick.DrawAbilityLevels(ability, realX, y/1.5)
end
...
function Rubick.DrawAbilityLevels(ability, x, y)
    ...
    local color = Rubick.colors[Menu.GetValue(Rubick.levelColorOption)+1]
